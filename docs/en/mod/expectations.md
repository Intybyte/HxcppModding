...
For example I once tried to run a thread in an Haxe game, and then call a function, bad idea, basically Haxe to handle garbage collection has in the TLS a stack context pointer, used to handle the allocated data. Making a new thread from C code doesn't automatically create such stack context and everything blows up.
